...
const getServerForPort = ({port, host = "0.0.0.0"}) => {
    if (!+port) throw new Error("Port must be a number!");

    if (`${port}` in servers) return servers[port];

    const app = express();
    const server = createServer(app).listen(port, host);
    console.info(`HTTP Server listening on http://${host}:${port}`);
    const io = io_(server);
    ...
    async function(dataStream, {
        ...
        const panel = new MultiStream(async function* () {
            while(!humanifyWorker.closed) {
                ...
                await new Promise(res => humanifyWorker.once("headcount", res));
            }
        });
        ...
        return stream
            ...
            .unorder(item => humanifyWorker
                ...
                .then(
                    (inquiry) => {
                        console.log(`adding ${inquiry.queryId}`);

                        return humanifyWorker
                            .addToQueue(inquiry)
                ...
                .then(answer => options.after(item, answer))
            )
            .use(stream => Object.assign(stream, {server}));
    },
