...
module.exports = class Humanify extends scramjet.DataStream {
            ...
            .then(
                (inquiry) => {
                    return this.sendToHumans(inquiry)
                        ...
                        .then(answerIdx => (console.error("A", inquiry), this.withdrawEntry(inquiry.queryId), inquiry.buttons[answerIdx] && inquiry.buttons[answerIdx].value));
                }
            )
            .then(
    ...
    waitUntilAnswered(id) {
        return new Promise(
            (res) => {
                ...
                console.error("waiting for " + id);
            }
        );
    }

    sendToHumans(inquiry) {
        console.error(inquiry.queryId + ": sending to humans");
        return new Promise((res) => {
            if (this.humanCount) {
                console.error(inquiry.queryId, "have humans");
                res();
            } else {
                console.error(inquiry.queryId, "wait foir humans");
                this.once("humans", res);
            }
        }).then(
            () => {
                console.error(inquiry.queryId, "sending");
    ...
    handleAnswer(id, answer) {
        console.error("answer", id, answer, this.inquiries.has(id));
    ...
    addHuman(conn) {
        ...
        new Promise((res) => {
            ...
            () => process.exit(127)         // for app debugging
        );
