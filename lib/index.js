...
module.exports = class Humanize extends scramjet.DataStream {
            ...
            .then(
                (inquiry) => {
                    return this.sendToHumans(inquiry)
                        ...
                        .then(answerIdx => (console.error("A", inquiry), this.withdrawEntry(inquiry.queryId), answerIdx));
                }
            )
            .then(
                (answer) => console.log('item resolved', answer)
            );
    }
    ...
    sendToHumans(inquiry) {
        console.log(inquiry.queryId + ": sending to humans");
        return new Promise((res) => {
            if (this.humanCount) {
                console.log(inquiry.queryId, "have humans");
                res();
            } else {
                console.log(inquiry.queryId, "wait foir humans");
                this.once("humans", res);
            }
        }).then(
            () => {
                console.log(inquiry.queryId, "sending");
